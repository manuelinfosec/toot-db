...
tootDB is not suitable for real-world use, but may be of interest to others learning about
database internals.

## Documentation

* [Architecture guide](docs/architecture.md): a guide to tootDB's architecture and implementation.

* [SQL examples](docs/examples.md): comprehensive examples of tootDB's SQL features.

* [SQL reference](docs/sql.md): detailed reference documentation for tootDB's SQL dialect.

* [References](docs/references.md): books and other research material used while building tootDB.
...
tootDB supports most common SQL features, including joins, aggregates, and ACID transactions.

## Architecture

[![tootDB architecture](./docs/images/architecture.svg)](./docs/architecture.md)

tootDB's architecture is fairly typical for distributed SQL databases: a transactional
...
[VSCode](https://code.visualstudio.com) provides a very intuitive environment for debugging tootDB.
...
2. Go to "Run and Debug" tab and select e.g. "Debug unit tests in library 'tootdb'".

3. To debug the binary, create a data directory:

    ```
    $ sudo mkdir /var/lib/tootdb && sudo chown $(whoami) /var/lib/tootdb
    ```

   And then selet "Debug executable 'tootdb'" under "Run and Debug".
